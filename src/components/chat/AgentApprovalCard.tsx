...
import { Bot, Check, ListChecks, Loader2, ShieldAlert, X } from 'lucide-react';
import { apiFetch } from '@/lib/api';
import { useAuth } from '@/context/AuthContext';
import { settleApprovalLocally, useAgentApprovalsFor, type PendingApproval } from '@/stores/agent-approvals';
...
function renderInlineCode(text: string): ReactNode[] {
  ...
  return parts.map((part, i) =>
    i % 2 === 1 ? (
      <code key={`c-${i}-${part}`} className="rounded bg-muted px-1 py-0.5 font-mono text-[11px]">
        {part}
      </code>
    ) : (
          ...
          >
            ...
            <div className="flex items-center gap-2 border-b border-amber-500/20 bg-amber-500/10 px-3 py-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-amber-500/20">
                <Bot className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" aria-hidden="true" />
              </span>
              <span className="text-xs font-semibold">
                {agentLabel}
                <span className="font-normal text-muted-foreground">
                  {isReply ? ' drafted a reply' : isChoice ? ' needs you to pick an option' : ' is waiting for your approval'}
                </span>
              </span>
              <span className="ml-auto flex items-center gap-1 text-[10px] uppercase tracking-wide text-amber-600/80 dark:text-amber-400/80">
                ...
                {a.risk || (isReply ? 'draft reply' : isChoice ? 'question' : 'approval')}
              </span>
            </div>
            ...
            <div className="px-3 py-2.5">
              {isReply ? (
                ...
                <>
              <div className="text-xs leading-relaxed">{renderInlineCode(a.summary)}</div>
              ...
              {isChoice ? (
                ...
                <div className="mt-2.5 space-y-1.5">
                  {a.options?.map((opt, i) => (
                    <button
                      ...
                      type="button"
                      disabled={isBusy}
                      onClick={() => void decide(a.approvalID, a.runID, true, opt)}
                    ...
                    >
                      ...
                      <span className="min-w-0 break-words font-medium">{opt}</span>
                    </button>
                  ))}
                  <div className="flex justify-end pt-0.5">
                    <button
                      type="button"
                      disabled={isBusy}
                      onClick={() => void decide(a.approvalID, a.runID, false)}
                ...
                <div className="mt-2.5 flex items-center gap-2">
                  <button
                    type="button"
                    disabled={isBusy}
                    onClick={() => void decide(a.approvalID, a.runID, true)}
...
function ReplyProposal({
  ...
  busy: boolean;
  onDecide: (approvalID: string, runID: string, approve: boolean, choice?: string, text?: string) => Promise<void>;
}) {
  const [text, setText] = useState(approval.replyText ?? '');
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-muted-foreground">Edit if you like, then send — or cancel and reply yourself.</p>
      <textarea
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={() => void onDecide(approval.approvalID, approval.runID, true, undefined, text)}
        ...
        <button
          type="button"
          disabled={busy}
          onClick={() => void onDecide(approval.approvalID, approval.runID, false)}
          className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/40 px-3 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-500/10 disabled:opacity-50 dark:text-red-400"
        >
          <X className="h-3.5 w-3.5" aria-hidden="true" />
          Cancel — I&apos;ll reply

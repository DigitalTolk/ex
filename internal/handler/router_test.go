...
func TestNewRouterDoesNotPanic(t *testing.T) {
	...
	router := NewRouter(authH, userH, channelH, convH, wsH, nil, nil, nil, nil, nil, nil, nil, nil, jwtMgr, nil, "*")
...
func TestRouterHealthEndpoint(t *testing.T) {
	jwtMgr := auth.NewJWTManager("test-secret", 15*time.Minute, 24*time.Hour)
	router := NewRouter(&AuthHandler{}, &UserHandler{}, &ChannelHandler{}, &ConversationHandler{}, &WSHandler{}, nil, nil, nil, nil, nil, nil, nil, nil, jwtMgr, nil, "*")

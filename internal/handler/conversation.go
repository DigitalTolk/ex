...
// SetNoUnfurl flips the link-preview suppression flag on a conversation
// message. Body: { "noUnfurl": bool }. Author-only.
func (h *ConversationHandler) SetNoUnfurl(w http.ResponseWriter, r *http.Request) {
	userID := middleware.UserIDFromContext(r.Context())
	id := pathParam(r, "id")
	msgID := pathParam(r, "msgId")
	if id == "" || msgID == "" {
	...
	msg, err := h.messageSvc.SetNoUnfurl(r.Context(), userID, id, service.ParentConversation, msgID, body.NoUnfurl)
	if err != nil {
...
// ListFiles returns every attachment ever shared in the conversation,
// newest first.
func (h *ConversationHandler) ListFiles(w http.ResponseWriter, r *http.Request) {
	userID := middleware.UserIDFromContext(r.Context())
	id := pathParam(r, "id")
	if id == "" {
		writeError(w, http.StatusBadRequest, "missing_id", "conversation ID is required")
	...
	files, err := h.messageSvc.ListFiles(r.Context(), userID, id, service.ParentConversation)
	if err != nil {
